...
@user_router.post("/")
async def create_user(user: User, async_session: AsyncSession = Depends(get_db)):
    return await UserUseCases.create_user(user, async_session)


@user_router.get("/")
async def get_users(async_session: AsyncSession = Depends(get_db)):
    return await UserUseCases.get_users(async_session)


@user_router.post("/batch")
async def create_users_batch(users: list[User], async_session: AsyncSession = Depends(get_db)):
