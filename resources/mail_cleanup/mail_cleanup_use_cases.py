...
class MailCleanupUseCases:
    ...
    async def cleanup_by_phones(cls,
        ...
        df = cls.get_csv_from_file(file_binary)
        cls.validate_dataframe(df=df)
        phone_rows = ("phone1", "phone2", "phone3", "phone4", "phone5")
        df.columns = ("cpf", *phone_rows)

        phones_to_remove_cpf = await cls.get_active_phones_to_remove(
    ...
    @classmethod
    async def get_active_phones_to_remove(cls, async_session, df, phone_rows, user_active_repository):
    ...
    def validate_dataframe(cls, df: pd.DataFrame):
        ...
        cpf_column = df[0]
        cpf_errors = cls.check_regex(cpf_column, CPF_REGEX)
        if len(cpf_errors):
        ...
        for i in range(1, 6):
            ...
            phone_errors = cls.check_regex(phone_column, PHONE_REGEX)
            if len(phone_errors):
                raise DataFrameRowsValidationException(
    ...
    @classmethod
    def get_csv_from_file(cls, file: bytes) -> pd.DataFrame:
        try:
    ...
    @classmethod
    def check_regex(cls, column: pd.Series, regex: str) -> dict:
